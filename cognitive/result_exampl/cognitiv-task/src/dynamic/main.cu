...
void runConsoleWithXMLOutput(int iterations, int outputFrequency){
	...
	for (int i=0; i< iterations || iterations == 0; i++)
	{
		...
		if((i+1) % outputFrequency == 0){
			saveIterationData(outputpath, i+1, get_host_vertice_default_agents(), get_device_vertice_default_agents(), get_agent_vertice_default_count());
	...
	if(iterations % outputFrequency != 0){
		saveIterationData(outputpath, iterations, get_host_vertice_default_agents(), get_device_vertice_default_agents(), get_agent_vertice_default_count());
		printf("Iteration %i Saved to XML\n", iterations);
	}
